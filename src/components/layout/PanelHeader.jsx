...
import { Menu, Bell, User, AlertTriangle, X, Calendar, Clock, CarFront, Wrench, Plus, Check, ShieldAlert } from 'lucide-react';
...
const PanelHeader = ({ sidebarOpen, setSidebarOpen }) => {
    ...
    const fetchNotifications = async () => {
        try {
            const res = await fetch('https://csdwindo.com/api/panel/data_booking.php?status=INDIKASI_KOMPLEN');
    ...
    useEffect(() => {
        ...
        fetchNotifications();
        const notificationTimer = setInterval(fetchNotifications, 30000); // Check every 30s
                        ...
                        {isDropdownOpen && (
                            <>
                                ...
                                <motion.div 
                                    ...
                                    className="absolute right-0 mt-2 w-80 bg-white shadow-2xl rounded-xl border border-[#E5E5E5] z-50 overflow-hidden"
                                >
                                    <div className="p-4 border-b border-[#E5E5E5] bg-gray-50 flex justify-between items-center">
                                        <h3 className="font-display font-bold text-xs uppercase tracking-widest text-[#111111]">Notifikasi Indikasi Komplen</h3>
                                        <span className="bg-[#E60012] text-white text-[10px] px-2 py-0.5 rounded-full font-bold">{notifications.length}</span>
                                    </div>
                                    <div className="max-h-[350px] overflow-y-auto">
                                        {notifications.length > 0 ? (
                                            ...
                                            <div className="p-8 text-center">
                                                ...
                                                <p className="text-xs text-gray-400 font-medium italic">Tidak ada indikasi komplen baru</p>
                                            </div>
                                        )}
                                    </div>
                                    {notifications.length > 0 && (
