...
import { Search, X, Loader2, Car, Calendar, User } from 'lucide-react';
...
const NPSSearchModal = ({ isOpen, onClose, onSelect, onSearchSubmit, filters }) => {
    ...
    useEffect(() => {
        ...
        const fetchResults = async () => {
            ...
            try {
                ...
                const res = await fetch(`https://csdwindo.com/api/panel/nps_detail.php?${params.toString()}`);
                const data = await res.json();
                if (active && data.status) setResults(data.data?.list || []);
                ...
                >
                    ...
                    <div className="relative border-b border-[#E5E5E5] p-4 flex items-center gap-3 bg-gray-50">
                        ...
                        <input
                            ...
                            placeholder="Cari nama, rangka, kendaraan, atau note..."
                                        ...
                                        >
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${getBadgeColor(item.status_nps)}`}>{item.status_nps} ({item.score})</span>
                                                </div>
                                                <div className="flex items-center gap-4 text-xs text-gray-500">
                                                    <span className="flex items-center gap-1">
                                                        <Car size={12} /> {item.kendaraan || '-'}
                                                    </span>
                                            ...
                                            <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-1 text-xs">
                                                <span className="font-bold text-[#E60012]">{item.rangka || '-'}</span>
                                            </div>
                                        </div>
                                    ))}
                                    {results.length > 10 && (
