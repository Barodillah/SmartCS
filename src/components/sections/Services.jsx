...
import { Calendar, Settings, Tag, Phone, Car } from 'lucide-react';
...
const servicesData = [
    ...
    },
    {
        ...
        img: "https://www.mitsubishi-motors.co.id/images/cars/pajero/promo-grid1.webp",
        icon: Car,
        cta: "Coba Sekarang"
    },
];
...
const Services = () => {
    ...
    const x = useTransform(scrollYProgress, [0, 1], ["0%", "-83.33%"]); // -83.33% for 6 panels (500/600)

    return (
        <section ref={targetRef} className="relative h-[600vh] bg-[#111111]">
                            ...
                            <div className="relative z-10 px-12 md:px-24 w-full">
                                <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row md:items-end justify-between gap-10">
                                    ...
                                    <div className="flex-shrink-0">
                                        <AngularButton variant="primary" className="!px-10 !py-4 !text-[14px]" onClick={() => window.dispatchEvent(new CustomEvent('openDinaChat', { detail: { message: `Saya tertarik dengan layanan ${service.title}` } }))}>
                                            {service.cta}
                                        </AngularButton>
                                    </div>
                                </div>
